...
write_elf_header(struct cache_data *cd_header)
{
	 ...
	 */
	e_phnum = num_loads_dumpfile >= PN_XNUM ? PN_XNUM : num_loads_dumpfile;
	//e_phnum = num_loads_dumpfile >= 64 ? PN_XNUM : num_loads_dumpfile;
	...
	if (is_elf64_memory()) { /* ELF64 */
		...
		offset_note_dumpfile = sizeof(ehdr32)
		    + sizeof(Elf32_Phdr) * num_loads_dumpfile;
	}
	...
	/* Write the initial section header if necessary. */
	if (e_phnum == PN_XNUM) {
		if (is_elf64_memory()) {
			Elf64_Shdr shdr64;
			...
			if (!write_buffer(info->fd_dumpfile,
					offset_note_dumpfile, &shdr64,
					sizeof(shdr64), info->name_dumpfile))
				goto out;
			...
			ehdr32.e_shoff = offset_note_dumpfile;
			ehdr32.e_shentsize = sizeof(shdr32);
			ehdr32.e_shnum = 1;
			ehdr32.e_shstrndx = SHN_UNDEF;

			memset(&shdr32, 0, sizeof(shdr32));
			shdr32.sh_type = SHT_NULL;
			shdr32.sh_size = ehdr32.e_shnum;
			shdr32.sh_link = ehdr32.e_shstrndx;
			shdr32.sh_info = num_loads_dumpfile;

			if (!write_buffer(info->fd_dumpfile,
					offset_note_dumpfile, &shdr32,
					sizeof(shdr32), info->name_dumpfile))
				goto out;

			offset_note_dumpfile += sizeof(shdr32);
		}
	}
